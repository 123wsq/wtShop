...
public class TitleItemDecoration extends RecyclerView.ItemDecoration {
    ...
    private static int TITLE_BG_COLOR = Color.parseColor("#f8f8f8");
    private static int TITLE_TEXT_COLOR = Color.parseColor("#a3a3a3");
    private static int mTitleTextSize;


    public TitleItemDecoration(Context context, List<ContactBean> data) {
        super();
        mData = data;
        mContext = context;
        mPaint = new Paint();
    ...
    private void drawTitle(Canvas c, int left, int right, View child, RecyclerView.LayoutParams params, int position) {
        ...
        mPaint.getTextBounds(mData.get(position).getName(), 0, mData.get(position).getLetters().length(), mBounds);

        c.drawText(mData.get(position).getLetters(),
                child.getPaddingLeft() + mTitleHeight / 2,
