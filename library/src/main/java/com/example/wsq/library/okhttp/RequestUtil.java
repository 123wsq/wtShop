...
import com.example.wsq.library.okhttp.ssl.HttpsUtils;
...
class RequestUtil {
    ...
    RequestUtil(String methodType, String url, Map<String, String> paramsMap, List<Map<String, Object>> files, Map<String, String> headerMap, CallBackUtil callBack) {
        this(methodType, url, paramsMap, files ,callBack);
    }
    ...
    private void setPostParameAndListMapFile() {
        if(mListFileMap != null){
            ...
            for (Map<String, Object> map : mListFileMap){
                ...
                builder.addFormDataPart(key,fileName, RequestBody.create(MediaType.parse(fileType), f));
            }
            mRequestBuilder.post(builder.build());
        }
    }
    ...
    private void setHeader() {
        if(mHeaderMap != null){
            for (String key: mHeaderMap.keySet()){
                ...
                LogUtils.d("请求头:  "+ key+":"+mHeaderMap.get(key) );
            }
        }
    }
